...
ZR_BUTTON = 7               # ZR trigger (run test below to confirm)
...
def pick_serial_port() -> str:
    ...
    if not cands:
        raise RuntimeError("No Arduino serial port found. Check /dev/ttyACM* or /dev/ttyUSB*")
    return cands[0]
...
def main():
    ...
    if js.get_numaxes() <= max(LEFT_Y_AXIS, RIGHT_Y_AXIS):
        raise RuntimeError(f"Controller has only {js.get_numaxes()} axes, need up to {max(LEFT_Y_AXIS, RIGHT_Y_AXIS)}.")
    ...
    try:
        while True:
            ...
            # Update "fresh input" timestamp
            any_button = False
            for b in range(min(js.get_numbuttons(), 24)):
                if js.get_button(b):
                    any_button = True
                    break
            if abs(ly) > 0.01 or abs(ry) > 0.01 or any_button:
                last_input_time = now

            # Optional emergency stop button
            if js.get_numbuttons() > STOP_BUTTON and js.get_button(STOP_BUTTON):
                tgt1 = 0
                tgt2 = 0
                tgt3 = 0

            # Failsafe if controller input goes stale
            if (now - last_input_time) > FAILSAFE_TIMEOUT:
                tgt1 = 0
                tgt2 = 0
                tgt3 = 0
